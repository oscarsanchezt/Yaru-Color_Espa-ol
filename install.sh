...
#Intro
  ...
  echo -e "${BBlu}TRADUCIDO POR OSOSOSO${RCol}"
  echo -e "${BYel}Presione cualquier tecla para iniciar el instalador o CTRL-C para salir en cualquier momento ...${RCol}"
...
do
  echo -e "${BYel}Quieres instalar el pack completo?${RCol}"
  echo -e "${Yel}Este contiene todos los temas, iconos , cursores y todos los colores.${RCol}"
  ...
  do
    case $ask_install_type in
      ...
      no)
        ...
        ;;
      *)
        echo -e "${Yel}Porfavor seleccione 1 o 2.${RCol}"
        ;;
    esac
  done

  if [ "$package_install" == "false" ]; then # If the user DON'T want to install everything...
    echo -e "${BYel}Okay, Que color quieres instalar?${RCol}" # Ask the user which color should be installed...
    package_color_menu "${colors[@]}" # calling my function
    main_color_set=true
    install_theme=true

    echo -e "${BYel}Quieres instalar un pack de iconos? y es asi de que color ${RCol}"
    ...
    do
      case $ask_install_icons in
        ...
        no)
          ...
          break ;;
        *)
          echo -e "${Yel}Entrada incorrecta,ingrese un número de la lista anterior...${RCol}"
          ;;
        esac
    done

    if [ "$install_icons" == "true" ] && [ "$change_icon_color" == "true" ]; then # If the user want's to install an icon pack...
      echo -e "${BYel}Que color quieres instalar para tus iconos?${RCol}"
...
# Creating a summary of things to do
    echo -e ""
    echo -e "${BYel}Okay,esto deberia de ser suficiente por ahora...${RCol}"
    echo -e "${Yel}Hare lo siguente por ti:${RCol}"

    if [ "$package_install" == "true" ]; then # If the whole packages should be installed...
      echo -e "${BGre} - instale el packete completo${RCol}"
      echo -e "${Gre}Esto incluye todo los temas , cursores , iconos y colores${RCol}"
    fi

    if [ "$package_install" == "false" ]; then # If just a single theme should be installed...
      echo -e "${BGre} - instale yaru-$package_color theme.${RCol}"
      if [ "$install_icons" == "true" ] && [ "$change_icon_color" == "true" ]; then # If a different icon pack than the selected color should be installed...
        ...
        echo -e "${BGre} - No instale ningun icono.${RCol}" # Or don't install any icons...
      fi
    fi

    echo -e "${BYel}Es correcto?${RCol}"
    ...
    do
      case $ask_list_correct in
        yes)
          echo -e "${Yel}¡Excelente! ¡Lo haré!${RCol}"
          start_install=true
          break ;;
        no)
          echo -e "${Yel}Está bien ... Empecemos de nuevo ...${RCol}"
          echo -e ""
          echo -e ""
          echo -e ""
          start_install=false
          ...
          break ;;
        *)
          echo -e "${BYel}¡Respuesta incorrecta!${RCol}"
          ;;
        esac
      done
...
while [ "$picking" == "false" ] && [ "$install" == "true" ] && [ "$install_done" == "false" ]; do # so the while loop begins...
  ...
  if [ "$isroot" == "true" ]; then
    echo -e "${BRed}¡Se han detectado privilegios de root!${RCol}"
    echo -e "${Yel}Esto no es un problema, solo quería que me dijeras esto ...${RCol}"
    theme_dir_def="/usr/share/themes"
    icon_dir_def="/usr/share/icons"
  fi

  echo -e ""
  echo -e "${BYel}¿Dónde quieres instalar los temas?${RCol}"
  echo -e ""
  echo -e "${BYel}Ingrese la ruta de su tema[$theme_dir_def]: ${RCol}" && read -e theme_dir # asking for the path, if nothing is entered by the user 'theme_dir_def' will be used - new variable: 'theme_dir'
  theme_dir="${theme_dir:-$theme_dir_def}"
  if [[ ! -d "$theme_dir" ]]; then # and if this directory doesn't exists, create it.
		echo -e "${Yel}$theme_dir no existe, creando ahora${RCol}"
		mkdir -p $theme_dir
	fi
  echo -e "${BYel}Ingrese la ruta de su ícono [$icon_dir_def]: ${RCol}" && read -e icon_dir # asking for the path, if nothing is entered by the user 'icon_dir_def' will be used - new variable: 'icon_dir'
  icon_dir="${icon_dir:-$icon_dir_def}"
  if [[ ! -d "$icon_dir" ]]; then # and if this directory doesn't exists, create it.
    echo -e "${Yel}$icon_dir no existe, creando ahora${RCol}"
    mkdir -p $icon_dir
  fi

  # as the echo says, copy the files
  echo -e "${BYel}Copiando archivos...${RCol}"
  ...
  echo -e "${BYel}HECHO!${RCol}"
  echo -e ""

  echo -e "${BYel}¿Quieres habilitar temas, íconos y otras cosas?${RCol}"
  ...
  do
    case $ask_enable in
      yes)
          ...
          break ;;
      no)
          echo -e "${Gre}Está bien, está bien. ¡Terminamos!${RCol}"
          install_done=true
          install=false
          picking=false
          enable=false
          ...
          break ;;
      *)
          echo -e "${Red}Entrada incorrecta!${RCol}"
          ;;
        esac
      done
...
while [ "$install" == false ] && [ "$picking" == "false" ] && [ "$enable" == "true" ]; do
  ...
  if [ "$package_install" == "true" ]; then
    echo -e "${BYel}¿Quieres habilitar un tema? En caso afirmativo, ¿qué variante?${RCol}" # Ask for "a" theme if the complete package was installed
  else
    echo -e "${BYel}¿Desea habilitar el tema en caso afirmativo, qué variante?${RCol}" # Ask for "the" theme if just one was installed
  fi
  ...
  if [ "$enable_theme" == "true" ] && [ "$package_install" == "true" ]; then # Ask for color if a theme should be enabled and the whole package was installed
    echo -e "${BYel}Bien, ¿de qué color debería ser?${RCol}"
  ...
  if [ ! -d $homedir/.local/share/gnome-shell/extensions/user-theme* ]; then # checking if the user-themes extension of gnome-shell is enabled, it's needed to be able to set a different gnome-shell theme (why, though?)
    echo -e "${Red}Solo quería preguntarle si debería habilitar un tema de gnome-shell ... \nPero la extensión de temas de usuario no está instalada. \nPor favor, instálela para poder configurar un tema de gnome-shell.${RCol}" #...and if not, just tell this to the user
    echo -e "${Red}Puede obtener esto usando 'sudo apt install gnome-shell-extensions' \ ni aquí: https://extensions.gnome.org/extension/19/user-themes/"
  else
    echo -e "${Gre}Extensión de temas de usuario detectada!${RCol}"
    if [ "$package_install" == "true" ]; then
      echo -e "${BYel}¿Desea habilitar un tema de shell en caso afirmativo, qué variante?${RCol}" # Ask for "a" shell theme if the complete package was installed
    else
      echo -e "${BYel}¿Desea habilitar el tema de shell en caso afirmativo, qué variante?${RCol}" # Ask for "the" shell theme if just one was installed
    fi
    ...
    if [ "$enable_shell" == "true" ] && [ "$package_install" == "true" ]; then # Ask for color if a shell theme should be enabled and the whole package was installed
      echo -e "${BYel}¿Qué color de caparazón quieres habilitar?${RCol}"
      package_color_menu "${colors[@]}" # Just using the function from the top of this script to make it short
  ...
  echo -e ""
  if [ "$package_install" == "true" ]; then
    echo -e "${BYel}¿Quieres habilitar un paquete de iconos?${RCol}" # Ask for "an" icon pack if the complete package was installed
  else
    echo -e "${BYel}¿Quieres habilitar el paquete de iconos?${RCol}" # Ask for "the" icon if just one was installed
  fi
  # aaand again...
  ...
  if [ "$enable_icons" == "true" ] && [ "$package_install" == "true" ]; then # Ask for color if an icon pack should be enabled and the whole package was installed
    echo -e "${BYel}¿Qué color de icono desea habilitar?${RCol}"
  ...
  echo -e ""
  if [ "$package_install" == "true" ]; then
    echo -e "${BYel}¿Quieres habilitar un tema de cursor?${RCol}" # Ask for "a" cursor theme if the complete package was installed
  else
    echo -e "${BYel}¿Quieres habilitar el tema del cursor?${RCol}" # Ask for "the" cursor theme if just one was installed
  fi
  ...
  if [ "$enable_cursor" == "true" ]  && [ "$package_install" == "true" ]; then # Ask for color if a cursor theme should be enabled and the whole package was installed
    echo -e "${BYel}¿Qué tema de cursor desea habilitar?${RCol}"
    icon_color_menu "${colors[@]}" # Just using the function from the top of this script to make it short
  ...
  if [ -d /usr/share/gnome-shell/extensions/ubuntu-dock@ubuntu.com ]; then # check if ubuntu dock is installed
    if [ "$package_install" == "true" ]; then
      echo -e "${BYel}¿Quieres establecer un color de indicador de muelle?${RCol}" # Ask for "a" cursor theme if the complete package was installed
    else
      echo -e "${BYel}¿Quieres configurar el indicador de muelle?${RCol}" # Ask for "the" cursor theme if just one was installed
    fi
    ...
    if [ "$enable_dock" == "true" ]  && [ "$package_install" == "true" ]; then # Ask for color if a cursor theme should be enabled and the whole package was installed
      echo -e "${BYel}¿Qué color desea para los indicadores de muelle?${RCol}"
      icon_color_menu "${colors[@]}" # Just using the function from the top of this script to make it short
  ...
  echo -e "${Yel}Está bien, haré lo siguiente por ti:\n${RCol}"
  # Theme part:
  if [ "$theme_color_set" == "true" ]; then # If package was installed and user selected a color...
    echo -e "${BGre} - ENABLE Yaru-$enable_theme_color$theme_variant (usuario seleccionado) GTK theme${RCol}"
    ...
    echo -e "${BRed} - no cargado GTK theme${RCol}"
  fi
  ...
  echo -e ""
  echo -e "${BYel}¿Es correcto este resumen?${RCol}"
...
while [ "$execute" == "true" ]; do
  ...
  sudo -u ${RUID} DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/${RUSER_UID}/bus" gsettings set org.gnome.shell.extensions.user-theme name "Yaru-$enable_shell_color$shell_variant"
    if [ ! "$?" -eq "0" ]; then # so, if the above command to change the shell theme doesn't work, ask if the script should fix this.
      echo -e "${BRed}ERROR, esto es causado por un error antiguo en la extensión. \ N¡No te preocupes! Puedo arreglarlo, pero necesito privilegios de root.${RCol}"
      echo -e "${BRed}¿Esto está bien para ti? Eche un vistazo al archivo Léame para obtener más información.${RCol}"
      select shell_error_fix in yes no
      do
        case $shell_error_fix in
          yes)
            echo -e "${Yel}Okay, I'll fix this.${RCol}"
            echo -e "${Yel}Ingrese su contraseña si se le solicita${RCol}"
            ...
            break ;;
          no)
            echo -e "${Yel}De acuerdo, omitiendo el tema de la concha. ¡Por favor, lea el archivo Léame para esto!${RCol}"
            enable_shell=false
            break ;;
          *)
            echo -e "${Red}Wrong input!${RCol}"
  ...
  sudo -u ${RUID} DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/${RUSER_UID}/bus" gsettings set org.gnome.shell.extensions.user-theme name "Yaru-$package_color$shell_variant"
    if [ ! "$?" -eq "0" ]; then # so, if the above command to change the shell theme doesn't work, ask if the script should fix this.
      echo -e "${BRed}ERROR, esto es causado por un error antiguo. ¡No te preocupes! \ NPuedo arreglarlo, pero necesito privilegios de root.${RCol}"
      echo -e "${BRed}¿Esto está bien para ti?${RCol}"
      select shell_error_fix in yes no
      do
        case $shell_error_fix in
          yes)
            echo -e "${Yel}Bien, arreglaré esto, por favor lea el archivo Léame para ver lo que hice.${RCol}"
            echo -e "${Yel}Por favor ingrese su contraseña ...${RCol}"
            # I've got this fix from here: https://gist.github.com/atiensivu/fcc3183e9a6fd74ec1a283e3b9ad05f0 - it puts the missing file from ~ to the glib-2.0 directory and re-compiles the glib schemas
            sudo cp $HOME/.local/share/gnome-shell/extensions/user-theme@gnome-shell-extensions.gcampax.github.com/schemas/org.gnome.shell.extensions.user-theme.gschema.xml /usr/share/glib-2.0/schemas && sudo glib-compile-schemas /usr/share/glib-2.0/schemas
            break ;;
          no)
  ...
  echo -e "${BYel}¡Todo debería estar hecho!${RCol}"
  echo -e "${BYel}¡Traducido al español pòr Osososo!${RCol}"
  echo -e "${Yel}Si falta algo, use la herramienta gnome-tweak.${RCol}"
  echo -e "${Yel}¡Eso es! ¡ADIÓS y GRACIAS!${RCol}"
  execute=false
